...
def expenditures(request):
    ...
    csv_url = reverse('2020:expenditures_csv') + "?"+ request.GET.urlencode()
...
def ies(request):
    ...
    csv_url = reverse('2020:ie_csv') + "?"+ request.GET.urlencode()
...
def committee(request, committee_id):
    ...
    return render(request, '2020/committee.html', context)
...
def inaugural(request):
    ...
    if not request.GET:
        return render(request, '2020/inaugural.html', {'form': form})
    ...
    page = request.GET.get('page')
    results = paginator.get_page(page)
    return render(request, '2020/inaugural.html', {'form':form, 'results':results})
